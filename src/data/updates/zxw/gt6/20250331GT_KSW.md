...
---
#### Summary
- New themes `KSW_BMW_ID8_GS_DOUBLE`, `KSW_BENZ_GLC` and `KSW_BENZ_FY3`
- Zlink app updated from `5.4.45` to `5.4.100`
- New "External mic output" factory setting
- Czech language

#### Changes
- New theme `KSW_BMW_ID8_GS_DOUBLE` with `id="53"` with a secondary screen support in the launcher, containing various information like live GPS data
- New theme `KSW_BENZ_GLC` with `id="54"` as a second theme so far with navigation bar support, like `KSW_BENZ_VITO`
- New theme `KSW_BENZ_FY3` with `id="55"` with blue, green, orange, purple, red and yellow options that looks to be relatively well-supported by most built-in apps. Seems to require `ALS_6208` client value and potentially have a lyrics functionality in the Music app.
- Initial changes to for upcoming `KSW_BMW_ID9` with `id="56"`, although it's not implemented in various apps yet
