...
---
Summary:
- New themes [`KSW_BMW_ID8_GS_DOUBLE`](/themes/zxw/53-ksw_bmw_id8_gs_double), [`KSW_BENZ_GLC`](/themes/zxw/54-ksw_benz_glc) and [`KSW_BENZ_FY3`](/themes/zxw/55-ksw_benz_fy3)
...
- New theme [`KSW_BMW_ID8_GS_DOUBLE`](/themes/zxw/53-ksw_bmw_id8_gs_double) with `id="53"` with a secondary screen support in the launcher, containing various information like live GPS data
- New theme [`KSW_BENZ_GLC`](/themes/zxw/54-ksw_benz_glc) with `id="54"` as a second theme so far with navigation bar support, like [`KSW_BENZ_VITO`](/themes/zxw/52-ksw_benz_vito)
- New theme [`KSW_BENZ_FY3`](/themes/zxw/55-ksw_benz_fy3) with `id="55"` with blue, green, orange, purple, red and yellow options that looks to be relatively well-supported by most built-in apps. Seems to require `ALS_6208` client value and potentially have a lyrics functionality in the Music app.
- Initial changes to for upcoming [`KSW_BMW_ID9`](/themes/zxw/56-ksw_bmw_id9) with `id="56"`, although it's not implemented in various apps yet
