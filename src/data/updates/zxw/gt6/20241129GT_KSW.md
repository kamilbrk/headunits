...
---
#### Summary
- New themes `KSW_BENZ_NTG7` and `KSW_BENZ_VITO`
- Czech language

#### Changes
- New theme `KSW_BENZ_NTG7` with `id="51"`
- New theme `KSW_BENZ_VITO` with `id="52"` with navigation bar, air conditioning data (°C/°F) and ambient light settings via an additional app
- New Ambient app (`com.szchoiceway.ambient`) for `KSW_BENZ_VITO` with "Breath", "Fade-in", "Flicker", "Musical rhythm" or "Steady on" modes, rhythmic sensitivity, colour and lightness settings
