...
---
#### Summary
- New theme `KSW_BMW_ID8_KJD` with ID `47`
- New theme `KSW_BENZ_LS` with ID `48` (under development)
...
#### Changes
- Theme `KSW_BMW_ID8_KJD` with `id="47"` introduced across most of the built-in apps with graphics and lots of duplicated code, mostly to handle its own images over the standard `KSW_BMW_ID8`
- Theme `KSW_BENZ_LS` with `id="48"` seems to be still under development as this firmware update does not have any graphics and custom logic for it yet
