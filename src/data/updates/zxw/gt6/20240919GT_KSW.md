...
---
Summary:
- New theme [`KSW_BMW_ID8_KJD`](/themes/zxw/47-ksw_bmw_id8_kjd) with ID `47`
- New theme [`KSW_BENZ_LS`](/themes/zxw/48-ksw_benz_ls) with ID `48` (under development)
...
- Theme [`KSW_BMW_ID8_KJD`](/themes/zxw/47-ksw_bmw_id8_kjd) with `id="47"` introduced across most of the built-in apps with graphics and lots of duplicated code, mostly to handle its own images over the standard [`KSW_BMW_ID8`](/themes/zxw/17-ksw_bmw_id8)
- Theme [`KSW_BENZ_LS`](/themes/zxw/48-ksw_benz_ls) with `id="48"` seems to be still under development as this firmware update does not have any graphics and custom logic for it yet
